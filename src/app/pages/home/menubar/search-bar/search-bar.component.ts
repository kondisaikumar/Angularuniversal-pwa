...
export class SearchBarComponent implements OnInit {
  @ViewChild('searchBar') inputEl: ElementRef;
  ...
  searchByText() {
    this.dataservice.searchByText = this.searchText.trim();
    this.dataservice.categoryId = '1,2,3,4';
    this.router.navigate(['/advance-filter'], { queryParams: { storeid: localStorage.getItem('storeId'), keyword: this.searchText.trim() } });
  }
  productRoute(spid) {
    if (spid !== 0) {
