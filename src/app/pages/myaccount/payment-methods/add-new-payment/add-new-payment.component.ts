...
export class AddNewPaymentComponent implements OnInit {
  ...
  constructor(
    ...
    this.store
      .select(ProductStoreSelectors.productStoreStateData)
      .subscribe((pssd) => {
        if (pssd) {
    ...
    this.customerService.getProfileDetails().subscribe((data: any) => {
      this.profile = data;
  ...
  ngOnInit() {
    this.returnUrl =
      ...
      "/myaccount/payment-methods";
    this.isSaveAddress = false;
    this.formAddNewPayment = this.formBuilder.group({
      firstName: [
        (this.customerInfo && this.customerInfo.FirstName) || this.FirstName,
        [Validators.required],
      ],
      lastName: [
        (this.customerInfo && this.customerInfo.LastName) || this.LastName,
        [Validators.required],
      ],
      sixteenDigitNumber: [
        "",
      ...
      phoneNumber: [
        (this.customerInfo && this.customerInfo.ContactNo) || this.ContactNo,
        [Validators.required],
      ],
    });
    this.getAddressList();
    this.mapsAPILoader.load().then(() => {
      ...
      autocomplete.addListener("place_changed", () => {
        this.ngZone.run(() => {
          ...
          this.formAddNewPayment.controls["address"].setValue(
            this.streetAddress
          );
          this.formAddNewPayment.controls["city"].setValue(this.locality);
          this.formAddNewPayment.controls["state"].setValue(
            this.administrative_area_level_1
          );
  ...
  getAddressList() {
    if (
      this.customerService.customerAddressList &&
      this.customerService.customerAddressList.ListAddress
    ) {
  ...
  showCardType(isValid) {
    ...
    if (isValid && cardNumber) {
      this.cardType = this.cardService.getCardType(
        this.formAddNewPayment.get("sixteenDigitNumber").value
      );
  ...
  get f() {
    return this.formAddNewPayment.controls;
  }
  ...
  onSaveCard() {
    ...
    this.cardProfile.phoneNumber = this.formAddNewPayment.get(
      "phoneNumber"
    ).value;

    this.cardProfile.cardNumber = this.formAddNewPayment.get(
      "sixteenDigitNumber"
    ).value;
    this.cardProfile.expirationDate = `${
      this.formAddNewPayment.get("expiryYear").value
    }-${this.formAddNewPayment.get("expiryMonth").value}`;
    ...
    if (
      ...
      this.cardProfile.customerProfileId =
        paymentMethodList.ListPaymentItem[0].UserProfileId;
    }
    if (
      paymentMethodList &&
    ...
    this.paymentService
      .cardValidPaymentProfileRequest(this.CardValidProfile)
      .subscribe((data) => {
        if (data && data.AuthResponse) {
          const cardApproveData = JSON.parse(data.AuthResponse);
          if(cardApproveData.transactionResponse && cardApproveData.transactionResponse.responseCode === '1'
           && paymentMethodList.ListPaymentItem[0].CardValidationAmount !== 0){
            this.paymentService.VoidTransactionRequest(cardApproveData.transactionResponse.transId).subscribe((data:any)=>{
              const dataRes= data;
            });
          }
          if(cardApproveData.transactionResponse && cardApproveData.transactionResponse.responseCode === '1'
           && cardApproveData.transactionResponse.avsResultCode === 'Y',
            cardApproveData.transactionResponse.cvvResultCode === 'M'){
  ...
  aftercardValidation() {
    ...
    if (this.cardProfile.customerProfileId) {
      this.paymentService
        .createCustomerPaymentProfileRequest(this.cardProfile)
        .subscribe((data) => {
          if (data && data.AuthResponse) {
            ...
            if (cardData.customerProfileId) {
              if (cardData.messages.resultCode === "Error") {
                if (
                  cardData.messages.message[0].text ===
              ...
              if (
                cardData &&
                cardData.messages &&
                cardData.messages.message &&
                cardData.messages.message.length > 0 &&
                cardData.messages.message[0].text
              ) {
                if (
                  cardData.messages.message[0].text ===
                  ...
                  this.paymentService
                    .createCustomerProfile(this.cardProfile)
                    .subscribe((recoveryres) => {
                      if (recoveryres && recoveryres.AuthResponse) {
                        const existingCardsrecovery = recoveryres.AuthResponse;
                        const cardDatarecovery = JSON.parse(
                          existingCardsrecovery
                        );
                        this.customerService
                          .customerPaymentInsert(
                            cardDatarecovery.customerProfileId,
                            true,
                            1
                          )
                          .subscribe((res) => {
      ...
      this.paymentService
        ...
        .subscribe((data) => {
          if (data && data.AuthResponse) {
            ...
            if (cardData.customerProfileId) {
              this.customerService
                .customerPaymentInsert(cardData.customerProfileId, true, 1)
                .subscribe((res) => {
              ...
              if (data && data.AuthResponse) {
                ...
                if (
                  cardDatafailure &&
                  cardDatafailure.messages &&
                  cardDatafailure.messages.message &&
                  cardDatafailure.messages.message.length > 0 &&
                  cardDatafailure.messages.message[0].text
                ) {
                  const str = cardDatafailure.messages.message[0].text;
                  const error = str.match(/\d/g);
                  if (error !== null && error !== undefined) {
                    this.toastr.error(
                      "Something went wrong. Please be patient we are recovering your previously saved cards if any."
                    );
                    const final = error.join("");
                    this.customerService
                      .customerPaymentInsert(final, true, 1)
                      .subscribe((res) => {
                        if (res && res.SuccessMessage !== "") {
                          this.toastr.success(res.SuccessMessage);
                          this.progressBarService.hide();
  ...
  saveAddress() {
    ...
    this.progressBarService.show();
    this.customerService.AddNewAddress(address).subscribe((data) => {
