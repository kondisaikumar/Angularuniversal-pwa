...
import { RecipeGetDetailRequestPayload } from '../models/recipe-get-details-request-payload';
@Injectable({
  providedIn: 'root'
})
export class RecipeService {
  ...
  getRecipeGetDetail(recipeid: any): Observable<any> {
    // tslint:disable-next-line:max-line-length
    return this.http.post<any>(baseUrl + UrlNames.RecipeGetDetail, this.getRecipeDetailRequestParams(recipeid), { headers: this.headers }).pipe(
      switchMap((res: any) => {
        return of(res);
      }),
      catchError((error: any, caught: Observable<any>) => {
    ...
    private getRecipeDetailRequestParams(recipeid: number): RecipeGetDetailRequestPayload {
      if (!this.customerSession) {
        return null;
      }
      return {
