...
if (res.IsProfileUpdate === true) {
  ...
  validateCaptcha(token: any, cat:any) {
    let body: any;
    let endpointUrl: any;
    const x = this.customerPaymentMethodGetList.ListPaymentItem.filter((item:any)=>item.PaymentTypeId !== 2)[0];
    if(x.PaymentTypeId === 1){
      body = {
        'tokenresponse': token,
      'StoreCaptchaKey':cat
      }
      endpointUrl = 'Store/ValidateCaptcha';
    } else if (x.PaymentTypeId === 7){
      body = {
        "StoreId": this.customerSession.StoreId,
        "AppId": this.customerSession.AppId,
        "SessionId":  this.customerSession.SessionId,
        ...
        endpointUrl = 'Customer/PaymentSetupTransactionIdByCaptcha';
    }
    return this.http.post<any>(baseUrl + endpointUrl,
      body, { headers: this.headers }).pipe(
        switchMap((res: any) => {
          this.TransactionSetupID = res.TransactionSetupID;
          return of(res);
        }),
